...
exports.default = function (PropTypes, extension) {
  ...
  var splitAssignments = function splitAssignments(children) {
    ...
    var isColonNode = function isColonNode(_ref) {
      ...
      return type === NODE_TYPE_LEAF && value === CHAR_ASSIGNMENT;
    };
  ...
  var buildExpressions = function buildExpressions(tokens) {
    var copy = [];

    for (var i = 0; i < tokens.length; i++) {
      var token = tokens[i];
      if (token.value === CHAR_REQUIRED) {
        var lastToken = copy.pop();
        if (!lastToken) {
    ...
    for (var _i = 0; _i < tokens.length; _i++) {
      ...
      if (_token.value === CHAR_UNION) {
        var _lastToken = copy.pop();
        var nextToken = tokens[_i + 1];
        if (!_lastToken || !nextToken) {
